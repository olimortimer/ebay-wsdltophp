...
class RefundTransactionInfoType extends AbstractStructBase
{
     ...
     * refund (RefundType=STORE_CREDIT). <br/> <span class="tablenote"> <strong>Note:</strong> For <strong>GetItemTransactions</strong>, <strong>GetSellerTransactions</strong>, and <strong>GetOrderTransactions</strong>, the final value fee amount deducted
     * from the seller's payout funds for the sale is not included in this field, which means that the amount in this field may not reflect the buyer's actual refund amount. <br/><br/> The logic will remain the same for <strong>GetItemTransactions</strong>,
     * <strong>GetSellerTransactions</strong>, and <strong>GetOrderTransactions</strong>, but the <strong>GetOrders</strong> call will be updated to include the the seller's final value fee amount in this field, so the amount in this field should match the
     * buyer's actual refund amount. To pick up this new logic in <strong>GetOrders</strong> responses, a Trading WSDL version of 1311 or above must be used, or the user can use an older Trading WSDL version but include and set the
     * <strong>X-EBAY-API-COMPATIBILITY-LEVEL</strong> header value to 1311 or above. </span>
     * - minOccurs: 0
